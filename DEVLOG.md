...
- Evaluated the PTY/ConPTY migration path for [src/terminal_session.rs](/d:/code/rustpilot/rustpilot/src/terminal_session.rs) and documented the staged backend plan in [docs/terminal-backend-evaluation.md](/d:/code/rustpilot/rustpilot/docs/terminal-backend-evaluation.md).
- Completed phase 1 of the terminal backend migration in [src/terminal_session.rs](/d:/code/rustpilot/rustpilot/src/terminal_session.rs): introduced an internal `LiveTerminalBackend` trait, moved the existing implementation behind a `PipeBackend`, and kept the public `TerminalManager` API and persistence behavior unchanged.
- Added a focused backend-level regression test for the phase-1 abstraction boundary: `PipeBackend` now explicitly reports resize as unsupported until a PTY backend exists.
- Started phase 2 of the terminal backend migration in [src/terminal_session.rs](/d:/code/rustpilot/rustpilot/src/terminal_session.rs): added a `PtyBackend` using `portable-pty`, made live session startup prefer PTY and fall back to `PipeBackend` if PTY startup fails, and kept the external terminal tool/session APIs unchanged.
- Added the `portable-pty` dependency in [Cargo.toml](/d:/code/rustpilot/rustpilot/Cargo.toml) and verified that the full existing test suite still passes with the PTY-capable backend path in place.
- Exposed the first PTY-specific capability through the public stack: [src/terminal_session.rs](/d:/code/rustpilot/rustpilot/src/terminal_session.rs) now supports manager-level `resize`, and [src/agent_tools.rs](/d:/code/rustpilot/rustpilot/src/agent_tools.rs) now exports `terminal_resize`.
- Extended [tests/app_integration.rs](/d:/code/rustpilot/rustpilot/tests/app_integration.rs) so the terminal tool lifecycle test now exercises `terminal_resize` through the real tool dispatch path.
...
- The PTY upgrade should begin with an internal backend trait and a `PipeBackend` wrapper, not with a direct rewrite of `TerminalManager`. That keeps the current session API, persistence model, and restored-session behavior stable while PTY/ConPTY support is introduced underneath.
- With the backend trait now in place, the next PTY step is implementation work rather than architectural cleanup: add a second live backend under the trait and decide whether rollout is opt-in or default.
- PTY support is now wired in behind the existing session layer, so the next meaningful step is capability exposure and validation: add explicit resize support at the manager/tool layer and add tests that prove PTY-specific interactive behavior beyond the old pipe baseline.
- With `terminal_resize` now exposed, the next high-value PTY work is behavior validation: add tests for prompt continuity and interactive programs that should behave better under PTY than under the old pipe backend.
